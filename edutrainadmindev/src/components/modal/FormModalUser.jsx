...
import { handleAddUserAccount } from "@/helpers/userAccountHelper";
import { toast } from "sonner";

export default function FormModalUser({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const response = await handleAddUserAccount(formData);
    setLoading(false);
    onClose();
    ...
    if (response === false) {
      toast.error("User Account Not Created");
    }
  };
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 p-8 text-primary-white bg-primary-blue rounded-lg shadow-md">
        ...
        <form onSubmit={handleSubmit}>
          <div className="pb-3">
            ...
            <input type="text" className="text-black w-full py-3 px-2 rounded-md outline-none mb-3" placeholder="Username" onChange={handleChange("username")} required />
